...
if [[ -d/docker/auto_spy_data/autospy  ]]; then
	docker stop auto_spy
	docker rm auto_spy
	echo "autospy配置文件已存在"
	docker run -dit --restart=always --name=auto_spy --log-opt max-size=100m --log-opt max-file=3 -v/docker/auto_spy_data/autospy:/autospy --hostname=auto_spy xieshang1111/auto_spy:$arch
else
	mkdir auto_spy_data
	...
	if [[ $dir != "" ]];then
	docker run -dit --restart=always --name=auto_spy --log-opt max-size=100m --log-opt max-file=3 -v/docker/auto_spy_data/autospy:/autospy --hostname=auto_spy xieshang1111/auto_spy:$arch
	cp $dir/auto_spy.yaml/docker/auto_spy_data/autospy
	docker exec -it auto_spy bash /autospy/start.sh
	exit 0
else
	mkdir auto_spy_data
	...
	docker rm auto_spy
	docker run -dit --restart=always --name=auto_spy --log-opt max-size=100m --log-opt max-file=3 -v/docker/auto_spy_data/autospy:/autospy --hostname=auto_spy xieshang1111/auto_spy:$arch
	cp/docker/auto_spy_data/autospy/auto_spy_simple.yaml/docker/auto_spy_data/autospy/auto_spy.yaml
